...
#predict CHLAMYDOSPORES model fit for each species 
ppredict_chlam <- function(data, model, lines=T, freex=F, prow=2, pcol=3, ...){
  
  #create new data to predict to and predict
  xx <- seq(-1,1.23,length.out = 100)
  newd <- data.frame(lesion=xx, area_samp=1, leafID=1, species=rep(unique(data$species), each=length(xx)))
  head(newd)
  newd <- newd %>% mutate(broad = ifelse(species %in% c('PIPO', 'PSME', 'SESE', 'LIDED', 'UMCAD'), 0, 1))
  pp <- fitted(model, newdata = newd, re_formula = ~(lesion|species), summary = F, scale = 'response')
  
  #define plot function
  plotmodfit <- function(i, pp){
    #need some of these vectors 
    sp <- sort(as.vector(unique(newd$species)))
    use <- as.integer(factor(newd$species, levels = sp))==i
    data$counte <- data$count/data$area_samp
    ymax <- data %>% group_by(species) %>% summarise(max(counte)*1.1) %>% as.data.frame()
    
    #plot it
    if(freex){ #range of lesion values
      plot(data$lesion[data$species==sp[i]], data$counte[data$species==sp[i]], xlab='Lesion area cm2', ylab='chlamydos', main=sp[i], pch=16, col=alpha('slateblue', .5), ylim=c(0,ymax[i,2]), ...)
      }else{ #lesions between 0 and 1.23
        plot(data$lesion[data$species==sp[i]], data$counte[data$species==sp[i]], xlab='Lesion area cm2', ylab='chlamydos', main=sp[i], pch=16, col=alpha('slateblue', .5), ylim=c(0,ymax[i,2]), xlim=c(0,1.23))
      }
    
    #shade or lines
    if(lines){
      draws <- sample(1:nrow(pp), 100)
      sapply(draws, function(x) {
        lines(xx, pp[x,use], lwd=.1, col=scales::alpha('black', .3))
      })
    }else{
      apply(pp[,use], 2, median) %>% lines(xx, .)
      apply(pp[,use], 2, HPDI, .9) %>% shade(., xx)
